...
static mxr_mutex_t os_execute_lock = MXR_MUTEX_INIT;
...
void *AcpiOsMapMemory(
    ...
    void* vaddr = NULL;
    // TODO(teisenbe): Replace this with something better; this only gets us
    // uncached memory
    mx_status_t status = mx_mmap_device_memory(aligned_address, end - aligned_address, &vaddr);
    if (status != NO_ERROR) {
        return NULL;
    }
...
ACPI_THREAD_ID AcpiOsGetThreadId() {
    return mxr_thread_get_handle(NULL);
}
...
static int acpi_os_task(void *raw_ctx) {
    ...
    mxr_mutex_lock(&os_execute_lock);
    os_execute_tasks--;
    if (os_execute_tasks == 0) {
        mx_event_signal(os_execute_event);
    }
    mxr_mutex_unlock(&os_execute_lock);
...
ACPI_STATUS AcpiOsExecute(
    ...
    mxr_mutex_lock(&os_execute_lock);
    os_execute_tasks++;
    mx_event_reset(os_execute_event);
    mxr_mutex_unlock(&os_execute_lock);

    mxr_thread_t* thread;
    mx_status_t status = mxr_thread_create(acpi_os_task, ctx, "acpi_os_exec", &thread);
    if (status != NO_ERROR) {
        free(ctx);
        mxr_mutex_lock(&os_execute_lock);
        os_execute_tasks--;
...
ACPI_STATUS AcpiOsCreateLock(ACPI_SPINLOCK *OutHandle) {
    mxr_mutex_t* lock = malloc(sizeof(mxr_mutex_t));
    if (!lock) {
        return AE_NO_MEMORY;
    }
    *lock = MXR_MUTEX_INIT;
...
ACPI_STATUS AcpiOsInstallInterruptHandler(
    ...
    mxr_thread_t* thread;
    mx_status_t status = mxr_thread_create(acpi_irq_thread, arg, "acpi_irq", &thread);
    if (status != NO_ERROR) {
        free(arg);
        return AE_ERROR;
    }
